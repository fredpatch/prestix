...
- Mounted modules: bootstrap, auth, users, settings, party, party-history, credit, documents (proformas/invoices/delivery-notes/payments), penalties/creances, stock, commissions, savings, reporting, audit-log, notifications
...
- Notifications backend: `/api/notifications` is authenticated in-app delivery with unread counts, list filters, read/archive actions, and system producers from document/jobs/commission workflows. Admin+ mail diagnostics live under `/api/notifications/mail/*`.
- Mail foundation: SMTP/Gmail delivery is centralized in `utils/mailer.ts`, guarded by environment configuration plus the Settings-backed `mail_enabled` switch, and persisted through the `mail_outbox` table for delivery history.
