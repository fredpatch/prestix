...
Cache/changelog sync after shared layout header polish.
...
- Party/Credit/History backend routes and Party client routes are drafted; document backend/client routes are mounted; payments backend routes and invoice-detail payment UI are drafted. Document creation now uses dedicated Proforma/Invoice pages, with Proforma creation refactored into a guided React Hook Form flow, and shared layout page headers replace duplicated in-page titles/back links. Header/create-flow smoke, penalty/creance runtime smoke, and legacy Beta cross-compare are the next gates.
