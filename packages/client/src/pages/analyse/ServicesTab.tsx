    ...
    <div>
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white border border-neutral-200 rounded-lg p-4">
          <p className="text-[11.5px] font-semibold text-neutral-800 mb-0.5">Service le plus utilisé</p>
          <p className="text-[10.5px] text-neutral-500 mb-3">Par volume de transactions, pas par valeur.</p>
          {byVolume.length === 0 || byVolume[0].volume === 0 ? (
            <p className="text-[11.5px] text-neutral-500 text-center py-8">Aucune donnée sur cette période.</p>
          ) : (
        ...
        <div className="bg-white border border-neutral-200 rounded-lg p-4">
          <p className="text-[11.5px] font-semibold text-neutral-800 mb-0.5">Service générant le plus de gain</p>
          ...
          <ChartCanvas
            ...
            config={{
              ...
              data: {
                ...
                datasets: [
                  { label: "Billetterie", data: trend.map((t) => t.billetterie), borderColor: SERIES_COLORS.billetterie, tension: 0.3 },
                  { label: "PrestiShop", data: trend.map((t) => t.prestishop), borderColor: SERIES_COLORS.prestishop, tension: 0.3 },
                  { label: "Commissions", data: trend.map((t) => t.commission), borderColor: SERIES_COLORS.commission, tension: 0.3 },
                  { label: "Épargne (frais)", data: trend.map((t) => t.epargne), borderColor: SERIES_COLORS.epargne, tension: 0.3 },
                  { label: "Pénalités", data: trend.map((t) => t.penalty), borderColor: SERIES_COLORS.penalty, tension: 0.3 },
                ],
              },
              options: {
                plugins: { legend: { position: "top", labels: { font: { size: 10.5 }, boxWidth: 10 } } },
                scales: {
