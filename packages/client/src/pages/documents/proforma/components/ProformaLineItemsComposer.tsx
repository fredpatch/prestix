...
function path(index: number, name: string) {
  return `lines.${index}.${name}` as FieldPath<ProformaFormValues>;
}

function ticketPath(index: number, name: string) {
  return `lines.${index}.ticketDetails.${name}` as FieldPath<ProformaFormValues>;
}

function firstSegmentPath(index: number, name: string) {
  return `lines.${index}.ticketDetails.segments.0.${name}` as FieldPath<ProformaFormValues>;
}

function referencePath(index: number, name: string) {
  return `lines.${index}.ticketDetails.references.${name}` as FieldPath<ProformaFormValues>;
}
...
function TicketFields({ index, line, error }: TicketFieldsProps) {
  const { register, setValue } = useFormContext<ProformaFormValues>();
...
export function ProformaLineItemsComposer() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useFormContext<ProformaFormValues>();
  ...
  function setLineType(index: number, value: "ticket" | "shop") {
    ...
    if (value === "ticket") {
      setValue(`lines.${index}` as FieldPath<ProformaFormValues>, {
        ...current,
        lineType: "ticket",
        quantity: 1,
