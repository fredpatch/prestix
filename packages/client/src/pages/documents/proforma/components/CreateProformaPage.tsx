...
import { proformaApi, type DocumentLineInput } from "@/lib/proforma.api";
...
function SummaryPanel({ values, isValid }: { values: ProformaFormValues; isValid: boolean }) {
  ...
  return (
    <aside className="border border-neutral-200 bg-white lg:sticky lg:top-6">
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="flex items-start gap-2 border-t border-neutral-200 pt-3 text-[11px] text-amber-700">
          <AlertTriangle size={14} className="mt-0.5 shrink-0" />
          <p>La proforma reste valable 48h à compter de sa création.</p>
        </div>
      </div>
    </aside>
  );
...
export default function CreateProformaPage() {
  ...
  const methods = useForm<ProformaFormValues>({
  ...
  async function submit(formValues: ProformaFormValues) {
    ...
    try {
      const res = await proformaApi.create(
        formValues.party.id,
        finalLines,
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="space-y-5">
            ...
            <div className="flex justify-end gap-2 border-t border-neutral-200 pt-4">
              <Button type="button" variant="secondary" onClick={() => navigate("/proformas")}>
                Annuler
              </Button>
              <Button type="submit" disabled={isSubmitting || !isValid}>
                {isSubmitting ? <Loader2 size={13} className="animate-spin" /> : "Creer"}
              </Button>
            </div>
          </div>
