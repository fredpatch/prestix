...
import { users, proformas } from "../../../db/schema.js";
...
function fmtDateTimeLong(d: Date | string): string {
...
export async function generateProformaPdf(
  ...
  const printData: PrintInvoiceData = {
    ...
    issueDate: fmtDateShort(proforma.createdAt),
    validUntil: proforma.expiresAt ? fmtDateTimeLong(proforma.expiresAt) : undefined,
    paymentMode: "-", // never applicable — proforma has no payments
